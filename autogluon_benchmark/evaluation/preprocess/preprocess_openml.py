...
def preprocess_openml_input(path, framework_suffix=None, framework_rename_dict=None, folds_to_keep=None, framework_suffix_column=None):
    ...
    if framework_suffix_column is not None:
        ...
        if 'framework_parent' in raw_input.columns:
            raw_input['framework_parent'] = [framework[0] + framework_suffix for framework in zip(raw_input['framework_parent'])]
        else:
    ...
    """
    cleaned_input['tid'] = cleaned_input['tid'].map(
        {
            360115: 360975,
            359947: 360945,
            14097: 360933,
...
def _rename_openml_columns(result_df):
    ...
    if 'score_test' in renamed_df.columns:
        print(f'score_test found in columns! Treating result as AG leaderboard format. score_test will be mapped to {METRIC_SCORE}, pred_time_test will be mapped to {TIME_INFER_S}.')
